...
var nodesReducer = function (previousState, action) {
    if (previousState === void 0) { previousState = (_a = {}, _a[constants_1.ROOT_NODE_ID] = [], _a); }
    var _a, _b, _c;
    if (action.meta && action.meta.optimistic) {
        if (action.meta.fetch === actions_1.MOVE_NODE) {
            return moveNode(previousState, action);
        }
        if (action.meta.fetch === ra_core_1.DELETE) {
            return removeNodes(previousState, [action.payload.id]);
        }
        if (action.meta.fetch === ra_core_1.DELETE_MANY) {
            return removeNodes(previousState, action.payload.ids);
        }
    }
    switch (action.type) {
        case actions_1.CRUD_GET_TREE_ROOT_NODES_SUCCESS: {
            var newState = __assign({}, previousState, (_b = {}, _b[constants_1.ROOT_NODE_ID] = action.payload.data.map(function (_a) {
                var id = _a.id;
                return id;
            }), _b), setupNodesChildren(previousState, action.payload.data));
            return newState;
        }
        case actions_1.CRUD_GET_TREE_CHILDREN_NODES_SUCCESS: {
...
var moveNode = function (previousState, action) {
    ...
    var previousParentId = action.payload.previousData[action.meta.parentSource] || constants_1.ROOT_NODE_ID;
    ...
    var newParentId = action.payload.data[action.meta.parentSource] || constants_1.ROOT_NODE_ID;
