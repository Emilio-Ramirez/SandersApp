...
exports.addRecords = function (newRecords, oldRecords) {
    ...
    var newFetchedAt = ra_core_1.getFetchedAt(newRecords.map(function (_a) {
        var id = _a.id;
        return id;
...
var dataReducer = function (previousState, _a) {
    ...
    if (meta && meta.optimistic) {
        if (meta.fetch === ra_core_1.UPDATE || meta.fetch === actions_1.MOVE_NODE) {
            var updatedRecord = __assign({}, previousState[payload.id], payload.data);
            var nextSiblings = meta.positionSource
                ? Object.keys(previousState)
                    .filter(function (id) {
                    return previousState[id][meta.parentSource] ===
                    ...
                    .map(function (id) {
                    var _a;
                    return (__assign({}, previousState[id], (_a = {}, _a[meta.positionSource] = previousState[id][meta.positionSource] + 1, _a)));
                })
                : [];
            return exports.addRecords([updatedRecord].concat(nextSiblings), previousState);
        }
        if (meta.fetch === ra_core_1.UPDATE_MANY) {
        ...
        if (meta.fetch === ra_core_1.DELETE) {
            return removeRecords([payload.id], previousState);
        }
        if (meta.fetch === ra_core_1.DELETE_MANY) {
            return removeRecords(payload.ids, previousState);
        }
    }
    if (!meta || !meta.fetchResponse || meta.fetchStatus !== ra_core_1.FETCH_END) {
        return previousState;
    }
    switch (meta.fetchResponse) {
        case actions_1.GET_TREE_ROOT_NODES:
        case actions_1.GET_TREE_CHILDREN_NODES:
